...
workflow annotate {
    ...
    main:
        ...
        post_processing_meta = hmmscan_viphogs.out.map{ name, set_name, _, faa  -> [name,set_name,faa]}.collect()
        viphogs_results = hmmscan_viphogs.out.map{ _, __, table, ___  -> table}.collect()
        rvdb_results = hmmscan_rvdb.out.map { _, __, table, ___  -> table}.collect()
        vogdb_results = hmmscan_vogdb.out.map { _, __, table, ___ -> table}.collect()
        pvogs_results = hmmscan_pvogs.out.map { _, __, table, ___ -> table}.collect()

        hmm_postprocessing(post_processing_meta, viphogs_results, rvdb_results, vogdb_results, pvogs_results)

        // calculate hit qual per protein
        ratio_evalue(hmm_postprocessing.out, params.custom_db)
...
workflow {
    ...
    if (params.fasta) {
      ...
      if (params.onlyannotate) {
        ...
        // )
        preprocess(fasta_input_ch)
        detect(preprocess.out, virsorter_db, virfinder_db, pprmeta_git)
        postprocess(detect.out)
        annotate(fasta_input_ch, postprocess.out, viphog_db, ncbi_db, rvdb_db, pvogs_db, vogdb_db, vpf_db, imgvr_db, additional_model_data, checkv_db, factor_file)
        plot(annotate.out)
      }
    } 
