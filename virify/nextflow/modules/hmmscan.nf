process hmmscan {
    ...
    if [[ ${params.db} == "viphogs" ]]; then
      if [[ ${params.version} == "v1" ]]; then
        hmmscan --cpu 2 --noali -E "0.001" --domtblout ${set_name}_${params.db}_hmmscan.tbl ${db}/${db}.hmm ${faa}
      else
        hmmscan --cpu 2 --noali --cut_ga --domtblout ${set_name}_${params.db}_hmmscan_cutga.tbl ${db}/${db}.hmm ${faa}
